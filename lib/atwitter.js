         ...
         function searchStep(path, cbEnd)
         {
            ...
            auth.get(r_options, function(err, jRes){
               if (err) cbEnd(err)
                  ...
                  if (jRes.errors) return cbEnd(new Error(jRes.errors[0].message));                  
                  if (jRes.error && jRes.error == "You have been rate limited. Enhance your calm.") return setTimeout(function(){searchStep(nextPath,searchCb);},30000);
